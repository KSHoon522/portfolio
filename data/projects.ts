...
export const workProjects: Project[] = [
  ...
  },
  {
    ...
    points: [
      "스마트 뉴스 — AI 보도자료 작성·첨삭, 부정보도 모니터링, SNS 댓글 분석·트렌드 차트 구현",
      "회의록 자동화 — mp4 스트리밍·VTT 자막·번역 스크립트, 리포트 생성·대시보드 시각화 화면 구현",
      "TipTap 리치 텍스트 에디터 기반 문서 작성 인터페이스 구축",
      "어시스턴트 응답 메타데이터·출처 신뢰도 임계값 파싱, 파일 업로드·다운로드 API 연동 — 착수 4주 내 UI 완성 후 API 연동",
    ],
    demoId: "editor",
  },
  {
    ...
    points: [
      "글로벌·부서·검색·AI 등 역할별 관리자 앱 분리, Provider 기반 페이지 접근 제한·사이드바 메뉴 제어 구현",
      "TypeScript 제네릭 기반 공통 테이블 컴포넌트를 설계해 17개 관리 화면에서 재사용 — 렌더 프롭·필터 주입·서버 페이지네이션 모드 지원",
      "Prometheus 쿼리 기반 모니터링 대시보드 — vLLM(토큰 처리량·GPU 메모리·캐시 사용률), 시스템(CPU/GPU), Elasticsearch 스택(노드·인덱스·스냅샷) 실시간 차트",
      "AI 채팅 프리뷰·이력 — SSE 스트리밍 타이핑, 출처 메타데이터 칩 파싱, 피드백(평점·코멘트·하이라이트), 파일 첨부, 응답 실패 시 무한 대기 버그 수정",
      "통합 검색 관리 — ECM·포털 소스별 검색 옵션, 하이라이트, 시맨틱 검색, CSV 내보내기, 검색 통계·응답시간 대시보드",
      "스케줄러(작업·대상 설정)·규정·프롬프트·부서·피드백 관리 화면 및 API 연동, 한/영 i18n, 정적 export·GitHub Actions 배포 구성",
    ],
    demoId: "admin-table",
  },
  {
